...
[`experiments/spongebob-guide/`](experiments/spongebob-guide/README.md). It began as an
...
steps, 38 annotated screenshots taken from the live Onshape UI, generated by the pipeline in
[`experiments/spongebob-guide/`](experiments/spongebob-guide/README.md). The reference model
builds 22/22 `OK`.
...
- **Session 1's two documents are reconciled.** `instructions/robot-guide/` supersedes
  ...
  *Carry the useful half of the old Session 1 into the new one*. The generated Sphinx guide in
  `experiments/spongebob-guide/` is the retired sponge pipeline.
...
- **Prose voice**: playful, never judging, never talking down. Numbers belong to *this SpongeBob*,
  never "mine" or "yours". Full rules in `experiments/spongebob-guide/content.py`'s docstring.
- **`experiments/spongebob-guide/content.py` is the single source of truth for words**; the
  generated `.rst` is not committed.
