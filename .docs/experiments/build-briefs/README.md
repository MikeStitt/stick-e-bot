...
`instructions/robot-guide/make_plans.py`, which holds them at module level so anything measuring
...
The sheets are generated by `instructions/robot-guide/make_brief_sheets.py` from the same
...
**Drive the browser through [`../../../tools/gui_steps.py`](../../../tools/gui_steps.py), not
through bare `page.mouse.click` calls.** Its `Steps` object takes each click with a sentence
saying what you are clicking, and writes the frame and both `steps.log` lines itself:
