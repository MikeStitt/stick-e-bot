...
All four are generated by `make_brief_sheets.py` from `make_plans.py`. Re-run `ninja brief-sheets`
if you doubt one.
...
**Build the whole list fresh.** draft9p4-check holds everything above `fork arm outline` and no
more.

**`relief slit` is the last feature on the blade.** Cut earlier, the stub axle bridges the two
leaves at the pin.
