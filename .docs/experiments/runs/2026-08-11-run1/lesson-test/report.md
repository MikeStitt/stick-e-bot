...
This also contradicts the lesson's own troubleshooting entry *"I cancelled a dimension with the red
...
**The lesson says:** "Click the slot's **centre line**, shift-click the **vertical line through the
...
**What worked instead** (labelled deviation): select the slot centreline's **two endpoints**,
shift-click the vertical line, and press **shift + q** for Symmetric. The mouth centred itself, and
...
then press **i** for **Coincident**. The rectangle snaps onto the centreline."
...
- `shift + q` on the two centre points plus the vertical line jumped them to matching distances;
...
its "origin — centre of the torso" callout remains the single most useful thing on either sheet.
...
4. **Sheet 2 carries an unresolved design question in red** — "the hip station sits on the torso's
   own bottom face, so the hip stud has nowhere to stand… Decide before the legs are modelled." That
...
Consistent with report 1, the time went where the text was incomplete, not where the modelling was
hard.
