#!/usr/bin/env python3
"""Read stickbot-draft9p1p6 back and check every settled number against make_plans.py.

Brought forward from ``../2026-09-04-draft9p1p5/check.py`` unchanged but for the ids and
this header. Every row already asks ``make_plans`` for its answer, so the twenty-four
wedge joint is read by the same rows that read the twelve wedge one.

One row is worth knowing about before it is read. ``crest_area`` clamps a crest's half
angle at zero, which was defensive on draft9p1p5 and is load bearing here: at 15 degrees
the two radial draft faces meet at r 7.478 and the crest is a ridge inboard of that, so
the integral genuinely runs to zero part way in.
...
the answer with the number ``make_plans`` derives.
    ...
    uv run --project . python -u .docs/experiments/runs/2026-09-08-draft9p1p6/check.py
"""
...
def check_ring(fs, t, c, face, up):
    ...
    regenerates, every station still reads right, and the crests merge into one disc on
    flanks that print as overhangs.
    """
