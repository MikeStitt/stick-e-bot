import sys
sys.path.insert(0, "/private/tmp/claude-501/-Users-mikestitt-projects-first-2027-sponge/c7e95b50-7b3c-4747-a656-ece9f33b51c4/scratchpad/b1")
import common
from playwright.sync_api import sync_playwright
import onshape_gui as gui
D = "/private/tmp/claude-501/-Users-mikestitt-projects-first-2027-sponge/c7e95b50-7b3c-4747-a656-ece9f33b51c4/scratchpad/b1/"
with sync_playwright() as p:
    browser, ctx, _ = gui.connect(p)
    page = common.mypage(ctx)
    gui.rename_row(page, "Linear pattern 1", "sole ribs"); page.wait_for_timeout(1200)
    gui.clear(page)
    page.keyboard.press("Shift+6"); page.wait_for_timeout(1200)
    gui.fit(page); page.wait_for_timeout(1200)
    gui.frame(page, D + "frames/cad.parts.foot.sole_ribs.png")
    page.keyboard.press("Shift+7"); page.wait_for_timeout(1200)
    gui.fit(page); page.wait_for_timeout(1200)
    gui.frame(page, D + "frames/cad.parts.foot.sole_groove.png")
    print([r for r in gui.tree(page)][-4:])
